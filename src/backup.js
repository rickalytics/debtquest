...
export function validateData(raw) {
  const fail = () => {
    throw new Error(
      "Invalid DebtQuest backup. Your existing data has not been replaced.",
    );
  };
  const object = (value) =>
  ...
  for (const a of data.accounts) {
    if (!str(a.name) || !str(a.type) || typeof a.isPaidOff !== "boolean")
      fail();
    for (const k of [
      "originalBalance",
      "currentBalance",
  ...
  for (const p of data.payments) {
    if (
      ...
      fail();
    for (const k of ["amountPaid", "balanceBefore", "balanceAfter"])
      if (!num(p[k])) fail();
  ...
  data.xp ??= 0;
  data.streak ??= 0;
  if (!num(data.xp) || !Number.isInteger(data.streak) || !num(data.streak))
    fail();
  data.achievements ??= [];
  if (!Array.isArray(data.achievements) || !data.achievements.every(str))
    fail();
  data.profile ??= { name1: "Player 1", name2: "Player 2" };
  if (
    !object(data.profile) ||
    !str(data.profile.name1) ||
    !str(data.profile.name2)
  )
    fail();
  if (
    data.lastPaymentMonth != null &&
    (typeof data.lastPaymentMonth !== "string" ||
      !/^\d{4}-\d{2}$/.test(data.lastPaymentMonth))
  )
    fail();
  if (data.journey !== undefined) {
    const j = data.journey;
    if (!object(j)) fail();
    for (const key of ["name", "why"])
      if (j[key] !== undefined && !str(j[key])) fail();
    for (const key of ["onboarded", "shareWins", "shareAmounts"])
      if (j[key] !== undefined && typeof j[key] !== "boolean") fail();
    if (j.lifetimeXP !== undefined && !num(j.lifetimeXP)) fail();
    if (
      j.style !== undefined &&
        ...
        (!Array.isArray(j[key]) ||
          ...
          !j[key].every(
            (value) =>
              ...
              new Date(value + "T12:00:00Z").toISOString().slice(0, 10) ===
                value,
          ))
      )
        fail();
