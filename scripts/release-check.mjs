...
requireCheck(
  ["local", "cloud"].includes(env.VITE_DATA_MODE),
...
if (env.VITE_DATA_MODE === "cloud") {
  requireCheck(
    /^https:\/\//.test(env.VITE_PUBLIC_APP_URL || ""),
    "Set the canonical HTTPS VITE_PUBLIC_APP_URL for circle invitations.",
  );
  requireCheck(
    /^https:\/\//.test(env.VITE_SUPABASE_URL || "") &&
      !!env.VITE_SUPABASE_ANON_KEY,
    "Set the existing Supabase project HTTPS URL and public anon/publishable key.",
  );
  ...
  requireCheck(
    role !== "service_role" && !key.startsWith("sb_secret_"),
    "A server secret cannot be bundled into the app. Replace it with the public key.",
  );
}
for (const path of [
...
if (errors.length) {
  ...
  console.log(
    "Automated preparation checks passed. Complete docs/app-store/RELEASE.md, device testing, hosted URLs, and Apple signing before submission.",
  );
